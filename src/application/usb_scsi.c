...
void SCSI_Inquiry_Cmd(u8 lun)
{
	...
	{
		Inquiry_Data = (lun == 0 ? Standard_Inquiry_Data : Standard_Inquiry_Data2);
		Inquiry_Data_Length = (CBW.CB[4] <= STANDARD_INQUIRY_DATA_LEN ? CBW.CB[4] : STANDARD_INQUIRY_DATA_LEN);
	}
...
void SCSI_RequestSense_Cmd(u8 lun)
{
	Transfer_Data_Request(
		Scsi_Sense_Data,
		(CBW.CB[4] <= REQUEST_SENSE_DATA_LEN ? CBW.CB[4] : REQUEST_SENSE_DATA_LEN)
		);
}
