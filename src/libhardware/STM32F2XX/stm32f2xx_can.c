...
uint8_t CAN_Transmit(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
	...
	if (transmit_mailbox != CAN_TxStatus_NoMailBox)
	{
		...
		else
		{
			...
			CANx->sTxMailBox[transmit_mailbox].TIR |= ((TxMessage->ExtId <<  3) | TxMessage->RTR  | TxMessage->IDE);
		}
		/* Set up the DLC */
...
FlagStatus CAN_GetFlagStatus(CAN_TypeDef* CANx, uint32_t CAN_FLAG)
{
	...
	{ 
		bitstatus = ((CANx->ESR & (CAN_FLAG & 0x000FFFFF)) != (uint32_t)RESET) ? SET :  RESET;
	}
	else if ((CAN_FLAG & CAN_FLAGS_MSR) != (uint32_t)RESET)
	{ 
		bitstatus = ((CANx->MSR & (CAN_FLAG & 0x000FFFFF)) != (uint32_t)RESET) ? SET : RESET;
	}
	else if ((CAN_FLAG & CAN_FLAGS_TSR) != (uint32_t)RESET)
	{ 
		bitstatus = ((CANx->TSR & (CAN_FLAG & 0x000FFFFF)) != (uint32_t)RESET) ? SET : RESET;
	}
	else if ((CAN_FLAG & CAN_FLAGS_RF0R) != (uint32_t)RESET)
	{ 
		bitstatus = ((CANx->RF0R & (CAN_FLAG & 0x000FFFFF)) != (uint32_t)RESET) ? SET : RESET;
	}
	else /* if (CAN_FLAG & CAN_FLAGS_RF1R != (uint32_t)RESET) */
	{ 
		bitstatus = ((uint32_t)(CANx->RF1R & (CAN_FLAG & 0x000FFFFF)) != (uint32_t)RESET) ? SET : RESET;
	}
	return  bitstatus;
}
...
ITStatus CAN_GetITStatus(CAN_TypeDef* CANx, uint32_t CAN_IT)
{
	...
	{	/* in case the Interrupt is enabled, .... */
		switch (CAN_IT)
		{
		case CAN_IT_TME:	/* Check CAN_TSR_RQCPx bits */
			itstatus = CheckITStatus(CANx->TSR, CAN_TSR_RQCP0|CAN_TSR_RQCP1|CAN_TSR_RQCP2);  
			break;
		case CAN_IT_FMP0:	/* Check CAN_RF0R_FMP0 bit */
			itstatus = CheckITStatus(CANx->RF0R, CAN_RF0R_FMP0);  
			break;
		case CAN_IT_FF0:	/* Check CAN_RF0R_FULL0 bit */
			itstatus = CheckITStatus(CANx->RF0R, CAN_RF0R_FULL0);  
			break;
		case CAN_IT_FOV0:	/* Check CAN_RF0R_FOVR0 bit */
			itstatus = CheckITStatus(CANx->RF0R, CAN_RF0R_FOVR0);  
			break;
		case CAN_IT_FMP1:	/* Check CAN_RF1R_FMP1 bit */
			itstatus = CheckITStatus(CANx->RF1R, CAN_RF1R_FMP1);  
			break;
		case CAN_IT_FF1:	/* Check CAN_RF1R_FULL1 bit */
			itstatus = CheckITStatus(CANx->RF1R, CAN_RF1R_FULL1);  
			break;
		case CAN_IT_FOV1:	/* Check CAN_RF1R_FOVR1 bit */
			itstatus = CheckITStatus(CANx->RF1R, CAN_RF1R_FOVR1);  
			break;
		case CAN_IT_WKU:	/* Check CAN_MSR_WKUI bit */
			itstatus = CheckITStatus(CANx->MSR, CAN_MSR_WKUI);  
			break;
		case CAN_IT_SLK:	/* Check CAN_MSR_SLAKI bit */
			itstatus = CheckITStatus(CANx->MSR, CAN_MSR_SLAKI);  
			break;
		case CAN_IT_EWG:	/* Check CAN_ESR_EWGF bit */
			itstatus = CheckITStatus(CANx->ESR, CAN_ESR_EWGF);  
			break;
		case CAN_IT_EPV:	/* Check CAN_ESR_EPVF bit */
			itstatus = CheckITStatus(CANx->ESR, CAN_ESR_EPVF);  
			break;
		case CAN_IT_BOF:	/* Check CAN_ESR_BOFF bit */
			itstatus = CheckITStatus(CANx->ESR, CAN_ESR_BOFF);  
			break;
		case CAN_IT_LEC:	/* Check CAN_ESR_LEC bit */
			itstatus = CheckITStatus(CANx->ESR, CAN_ESR_LEC);  
			break;
		case CAN_IT_ERR:	/* Check CAN_MSR_ERRI bit */ 
			itstatus = CheckITStatus(CANx->MSR, CAN_MSR_ERRI); 
			break;
		default:			/* in case of error, return RESET */
			itstatus = RESET;
			break;
		}
	}
